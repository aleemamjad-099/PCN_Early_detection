...
# """
# PCN Early Detection System — Professional Frontend v3.0
# Pancreatic Cancer & Neurodegenerative Disorder Detection
# Final Year Project | Medical AI
# """
...
#     BTN_SHADOW="0 4px 18px rgba(79,142,247,0.45)"
# else:
...
#     BTN_SHADOW="0 4px 18px rgba(29,78,216,0.30)"
...
# .ptag-b{{color:{BLUE};background:{BLUE_LT};border-color:{BLUE_MD};}}
# .ptag-t{{color:{TEAL};background:{TEAL_LT};border-color:{TEAL}44;}}
# .ptag-m{{color:{TXT_M};background:transparent;border-color:{BORDER2};}}
...
# .sbar{{border-radius:12px;padding:0.7rem 1.3rem;font-size:0.8rem;font-weight:500;display:flex;align-items:center;gap:0.65rem;margin-bottom:1.8rem;border:1px solid;font-family:'IBM Plex Sans',sans-serif;}}
...
# .stNumberInput > div > div > input{{background:{INP}!important;border:1.5px solid {BORDER2}!important;border-radius:10px!important;color:{TXT}!important;font-family:'IBM Plex Sans',sans-serif!important;font-size:0.95rem!important;font-weight:500!important;transition:border-color .18s,box-shadow .18s!important;padding:0.55rem 0.9rem!important;}}
...
# .ftbl th{{background:{CARD2};color:{TXT_M};font-weight:700;text-transform:uppercase;letter-spacing:0.1em;font-size:0.65rem;padding:0.7rem 1rem;text-align:left;border-bottom:1.5px solid {BORDER};font-family:'IBM Plex Sans',sans-serif;}}
...
# .disc{{background:{AMBER_LT};border:1px solid {DISC_BORD};border-radius:12px;padding:1rem 1.3rem;font-size:0.77rem;color:{DISC_TXT};line-height:1.65;margin-top:1.5rem;font-family:'IBM Plex Sans',sans-serif;}}
...
#         title="PCN Early Detection Clinical Report")

#     CB  = colors.HexColor("#1d4ed8"); CDB = colors.HexColor("#1e3a8a")
#     CR  = colors.HexColor("#b91c1c"); CG  = colors.HexColor("#15803d")
#     CS  = colors.HexColor("#334155"); CLG = colors.HexColor("#f1f5f9")
#     CMG = colors.HexColor("#e2e8f0"); CDG = colors.HexColor("#64748b"); CW  = colors.white
...
#     RC_BD    = colors.HexColor("#fecaca") if is_high else colors.HexColor("#bbf7d0")
#     prob_pct = result["probability_percentage"]
#     conf     = result["confidence"]
#     lat      = result["processing_time_ms"]
#     prob_raw = result["probability_raw"]
...
#     def TS():
#         return TableStyle([
#             ("BACKGROUND",(0,0),(-1,0),CB),("TEXTCOLOR",(0,0),(-1,0),CW),
...
#         Paragraph('<font name="Helvetica-Bold" size="17" color="#1e3a8a">PCN Early Detection System</font><br/>'
#                   '<font name="Helvetica" size="8.5" color="#64748b">Pancreatic Cancer &amp; Neurodegenerative Disorder · Clinical Assessment Report</font>', base["Normal"]),
#         Paragraph(f'<font name="Helvetica" size="7.5" color="#94a3b8">'
#                   f'Date: {datetime.datetime.now().strftime("%d %b %Y")}<br/>'
#                   f'Time: {datetime.datetime.now().strftime("%H:%M:%S")}<br/>'
#                   f'Report ID: PCN-{datetime.datetime.now().strftime("%Y%m%d%H%M%S")}</font>', RT),
#     ]], colWidths=[1.8*cm, 10*cm, 4.5*cm])
#     hdr.setStyle(TableStyle([
#         ("VALIGN",(0,0),(-1,-1),"MIDDLE"),("LINEBELOW",(0,0),(-1,0),1.5,CB),
#         ("BACKGROUND",(0,0),(0,0),colors.HexColor("#eef2ff")),
#         ("PADDING",(0,0),(-1,-1),7),("ALIGN",(0,0),(0,0),"CENTER"),
#     ]))
...
#         Paragraph(f'<font name="Helvetica-Bold" size="30" color="{col_hex}">{prob_pct}</font><br/>'
#                   f'<font name="Helvetica" size="8.5" color="#64748b">Combined PCN risk probability</font>',
#                   PS("PB",alignment=TA_CENTER)),
#         Paragraph(f'<font name="Helvetica-Bold" size="7.5" color="#334155">CONFIDENCE</font><br/>'
#                   f'<font name="Courier-Bold" size="9.5" color="#1d4ed8">{conf}</font><br/><br/>'
#                   f'<font name="Helvetica-Bold" size="7.5" color="#334155">THRESHOLD</font><br/>'
#                   f'<font name="Courier-Bold" size="9.5" color="#b45309">40%</font><br/><br/>'
#                   f'<font name="Helvetica-Bold" size="7.5" color="#334155">LATENCY</font><br/>'
#                   f'<font name="Courier-Bold" size="9.5" color="#64748b">{lat} ms</font>',
#                   PS("STK",alignment=TA_CENTER)),
#     ]], colWidths=[4.5*cm,6.5*cm,4.5*cm])
#     rbox.setStyle(TableStyle([
#         ("BACKGROUND",(0,0),(-1,-1),RC_BG),("BOX",(0,0),(-1,-1),1.8,RC),
...
#     pdata = [
#         [Paragraph('<font color="white"><b>Parameter</b></font>',MONO),Paragraph('<font color="white"><b>Value</b></font>',MONO),Paragraph('<font color="white"><b>Parameter</b></font>',MONO),Paragraph('<font color="white"><b>Value</b></font>',MONO)],
...
#     ]
#     pt = Table(pdata, colWidths=[4.2*cm,3.5*cm,4.2*cm,3.5*cm], repeatRows=1)
#     pt.setStyle(TS()); story += [pt, Spacer(1,14)]

#     # Engineered features
#     story += SEC("⚗️  Engineered Biomarker Features")
#     edata = [
#         [Paragraph('<font color="white"><b>Feature</b></font>',MONO),Paragraph('<font color="white"><b>Formula</b></font>',MONO),Paragraph('<font color="white"><b>Value</b></font>',MONO)],
...
#     ]
#     et = Table(edata, colWidths=[5*cm,7.2*cm,3.2*cm], repeatRows=1)
#     et.setStyle(TS()); story += [et, Spacer(1,14)]
...
#     bcols  = ["#1d4ed8","#0f766e","#7c3aed","#b45309","#db2777"]
#     max_v  = max(vals) if max(vals)>0 else 1
#     DW,DH  = 430,130
#     d = Drawing(DW,DH)
#     bw=50; gap=26; sx=35; base_y=28; ch=78
#     for i,(lb,v,bc) in enumerate(zip(lbls,vals,bcols)):
#         bx = sx+i*(bw+gap); bh = max(3,int((v/max_v)*ch))
#         d.add(Rect(bx,base_y,bw,bh,fillColor=colors.HexColor(bc),strokeColor=None))
#         d.add(String(bx+bw/2,base_y+bh+4,f"{v:.2f}",fontSize=7,textAnchor="middle",fontName="Helvetica-Bold",fillColor=colors.HexColor("#334155")))
#         d.add(String(bx+bw/2,8,lb,fontSize=6.5,textAnchor="middle",fontName="Helvetica",fillColor=colors.HexColor("#64748b")))
#     d.add(Line(sx-5,base_y,DW-15,base_y,strokeColor=CMG,strokeWidth=0.5))
#     story += [d, Spacer(1,12)]

#     # Gauge bar
#     pct_int = int(prob_raw*100); filled = int(pct_int/5)
#     story += SEC("Risk Probability Gauge")
#     filled_bar = "=" * filled + "-" * (20 - filled)
#     story.append(Paragraph(
#         f'<font name="Courier-Bold" size="10" color="{col_hex}">[{filled_bar}] {pct_int}%</font>',
#         base["Normal"]))
...
#     if is_high:
#         txt = (f"The patient demonstrates a <b>HIGH RISK</b> probability of {prob_pct} for combined "
#                "pancreatic cancer and neurodegenerative disorder, exceeding the 40% threshold. "
#                "Immediate specialist referral with imaging (CT/MRI), tumor markers (CA 19-9), and neurological evaluation is strongly advised.")
#     else:
#         txt = (f"The patient demonstrates a <b>LOW RISK</b> probability of {prob_pct} for combined "
#                "pancreatic cancer and neurodegenerative disorder, below the 40% threshold. "
#                "Standard follow-up per clinical guidelines recommended. Monitor metabolic and neurological markers routinely.")
#     story += [Paragraph(txt, BODY), Spacer(1,14)]

#     # Disclaimer
#     story += SEC("⚠️  Disclaimer")
#     dbox = Table([[Paragraph(
#         "<b>IMPORTANT:</b> This report is generated by an AI model trained on synthetic EHR and is intended "
#         "solely for research and decision-support purposes. Results must be reviewed by a licensed oncologist "
#         "or neurologist. This does not constitute a standalone medical diagnosis.", DISC)
#     ]], colWidths=[15.1*cm])
#     dbox.setStyle(TableStyle([("BACKGROUND",(0,0),(-1,-1),colors.HexColor("#fffbeb")),("BOX",(0,0),(-1,-1),1,colors.HexColor("#fde68a")),("PADDING",(0,0),(-1,-1),10)]))
#     story += [dbox, Spacer(1,12)]

#     # Footer
#     foot = Table([[Paragraph(
#         f'<font name="Helvetica" size="7" color="#94a3b8">'
#         f'PCN Early Detection System · Final Year Project · Synthetic EHR · ML v2.0 · XGBoost · FastAPI · Streamlit<br/>'
#         f'Generated: {datetime.datetime.now().strftime("%d %B %Y at %H:%M:%S")} · '
#         f'Report ID: PCN-{datetime.datetime.now().strftime("%Y%m%d%H%M%S")}</font>',
...
#         st.error(f"API error: {result.get('detail','Unknown')}")
#         st.stop()
...
#     tab1,tab2,tab3,tab4 = st.tabs(["  📊  Result Overview  ","  🧬  Feature Analysis  ","  📈  Biomarker Charts  ","  📋  Clinical Report  "])
...
#                     if is_high else
#                     "Risk is within acceptable range. Routine monitoring per clinical guidelines is advised.")
#             st.markdown(f"""
#             <div class="rcard {cls}">
...
#                 gauge={
#                     "axis":{"range":[0,100],"tickcolor":TICK,"tickfont":{"color":TICK,"size":9},"tickwidth":1,"dtick":20},
#                     "bar":{"color":gauge_col,"thickness":0.2},
#                     "bgcolor":GAUGE_BG,"borderwidth":0,
#                     "steps":[
#                         {"range":[0,40],"color":"#f0fdf4" if not D else "rgba(74,222,128,0.06)"},
...
#             ))
#             gauge.update_layout(paper_bgcolor=PLY_PAPER,plot_bgcolor=PLY_BG,
#                 height=280,margin=dict(l=20,r=20,t=60,b=10),font_color=TXT)
#             st.plotly_chart(gauge,use_container_width=True)
#         st.markdown(f'<div class="disc"><strong>⚠ Clinical Disclaimer:</strong> This tool provides decision support based on synthetic EHR-trained models. Results must be reviewed by a licensed oncologist or neurologist. Not intended for standalone diagnostic use.</div>', unsafe_allow_html=True)
...
#                 line=dict(color=BLUE,width=2.5),fillcolor=f"rgba({'79,142,247' if D else '29,78,216'},0.12)",
...
#             html='<table class="ftbl"><thead><tr><th>Feature</th><th>Formula</th><th>Value</th></tr></thead><tbody>'
#             for nm,formula,val in rows:
#                 html+=f'<tr><td><strong>{nm}</strong></td><td class="fmono">{formula}</td><td class="fval">{val}</td></tr>'
#             html+="</tbody></table>"
...
#         c="high" if h["label"]=="High Risk" else "low"
#         ic="⚠" if c=="high" else "✓"
#         html+=f'<div class="hitem"><span style="font-family:\'IBM Plex Mono\',monospace;font-size:0.7rem;color:{TXT_M};min-width:55px">#{i+1} {h["time"]}</span><div class="hbadge {c}">{ic} {h["label"]}</div><div><div class="hprob">{h["pct"]}</div><div class="hmeta">Age {h["age"]} · BMI {h["bmi"]:.1f}</div></div></div>'
#     st.markdown(html, unsafe_allow_html=True)
...
# st.markdown(f"""
# <div style="text-align:center;color:{TXT_M};font-size:0.7rem;font-family:'IBM Plex Sans',sans-serif;letter-spacing:0.05em;border-top:1px solid {BORDER};padding-top:1.5rem;line-height:2.2;">
#     PCN Early Detection System &nbsp;·&nbsp; Pancreatic Cancer &amp; Neurodegenerative Disorder &nbsp;·&nbsp; Final Year Project<br>
#     FastAPI &nbsp;·&nbsp; XGBoost &nbsp;·&nbsp; Streamlit &nbsp;·&nbsp; Plotly &nbsp;·&nbsp; ReportLab &nbsp;·&nbsp; {datetime.datetime.now().year}
# </div>
# """, unsafe_allow_html=True)
